<?php
		...
		echo '<div class="memberl"><ul id="NewsList" style="min-height: 0px;">';
		$nb = count($posts);
		$page = ceil($nb/5);
		if(empty($canshu['page'])){
			...
			foreach($posts as $p){
        		...
        		<a href="/member/member?id='.$p->ID.'"><img src="'.$sub_image.'"></a>
        	<dl>
        	<dt><p><a href="/member/member?id='.$p->ID.'">'.$p->post_title.'</a></p><span></span></dt>
            <dd>'.$des.'</dd>
        	</dl>
        	</li>';
				}
