...
!function (e, t) {
    ...
    function p() {
        var e = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : document, t = function (t) {
            if (c(t.origin) && t.data && "spacechange" === t.data.event) for (var n = e.querySelectorAll("iframe"), r = 0; r < n.length; r++) if (n[r].contentWindow === t.source) {
        ...
        T = (e(function (e, t) {
            !function (e) {
                ...
                function i(e, t) {
                    ...
                    if (t != t || 0 === t) for (g = e.length; g-- && !k(e[g], t);) ; else g = e.indexOf(t);
                    return -1 < g
                }
                ...
                var o = new T(function (r, o) {
                    var a = function (e) {
                        if (c(e.origin) && t.element.contentWindow === e.source) {
                            ...
                            var n = y(e.data), o = "event" in n && "ready" === n.event,
...
jQuery(document).ready(function () {
     ...
     */
    jQuery('#ga-optout').on('click', function (e) {
        e.preventDefault();
        ...
        switch (jQuery('html').attr('lang')) {
            case 'de':
                message = 'Das Google Opt-Out Cookie wurde gesetzt';
                break;
            case 'en':
            default:
                message = 'The Google Opt-Out Cookie has been set';
                break;
        }
    ...
    /**
     * Get a specific parameter from the queryString
     * @param {string} name
     * @return {string}
     */
    ...
    //To get how far the user has scrolled, use console.log($(window).scrollTop());
    if (!Modernizr.objectfit) {
        jQuery('.object_fit_image').each(function () {
        ...
        jQuery('.home_slider_item .home_slider').removeClass('ie_hide');
    }

    //Unchecks the filter buttons for news slider variation 2 on page load
    var params = getParameterByName('filter');
    var chk_box = params ? jQuery('.news-slider-wrap-var2 .filter-chkbox ul').find('#' + params.replace(',', ',#')) : jQuery('.news-slider-wrap-var2 .filter-chkbox ul .filter-chkbx');
    jQuery('.news-slider-wrap-var2 .filter-chkbox ul .filter-chkbx').prop('checked', false);
    chk_box.prop('checked', true);


    //Added this to make the content boxes slider images square always
    function square_content_boxes(window_width) {

        // When this function is called, First the desktop version runs and then the mobile version
        // This might be causing the problem.
        // So make it so that on mobile only the mobile version runs
        // and on desktop only the desktop version runs
        if (window_width >= 768) {
            setTimeout(function () {
                var $imgwidth;
                var b_height = 0;
                var b_height_set = 0;
                jQuery('.contbx').each(function () {
                    $sectioncontainer = jQuery(this);
                    if ($sectioncontainer.find('.smlbx')) {
                        $sectioncontainer.find('.smlbx').each(function () {
                            $smallboxcontainer = jQuery(this);
                            $imgwidth = $smallboxcontainer.css('width');
//                  $smallboxcontainer.find('img').attr('height', $imgwidth);
                            $smallboxcontainer.find('img').height($imgwidth);
                        });
                    }
                    b_height = $sectioncontainer.find('.smlbx').innerHeight();
                    b_height_set = b_height;

                    $sectioncontainer.parents('.hist_desk').find('.bigbx img').height(b_height_set);
                });
            }, 100);
        }
        //For mobile version
        if (window_width <= 768) {
            setTimeout(function () {
                var mobimgwidth = 0;
                var mobbigboxheight = 0;
                jQuery('.contbxmob').each(function () {
                    $sectioncontainer = jQuery(this);
                    if ($sectioncontainer.find('.smlbx')) {
                        $sectioncontainer.find('.smlbx').each(function () {
                            $smallboxcontainer = jQuery(this);
                            mobimgwidth = $smallboxcontainer.css('width');
                            //console.log("mobimgwidth:::");
                            //console.log(mobimgwidth);
                            $smallboxcontainer.find('img').height(mobimgwidth);
                        });
                        mobbigboxheight = $sectioncontainer.find('.smlbx').innerHeight();
                        //console.log("mobbigboxheight:::");
                        //console.log(mobbigboxheight);
                    }
                    $sectioncontainer.parents('.hist_mob').find('.bigbx img').height(mobbigboxheight);
                });
            }, 100);
        }
    }

    square_content_boxes(jQuery(window).width());
    ...
    jQuery('.search_div .search_results_headline, .search_div .search_results').remove();
    create_slider_wrap_slider();
    create_slider_wrap_slider_mobile();

    onNewsCheckboxClick(false);
    create_news_row_Var2_slider();
    create_news_row_Var2_slider_mobile();

    create_search_row_Var2_slider(jQuery('#search_fiter .searchdivs').length); /* getting count of the search results */

    jQuery('.search_results_headline .filter-chkbox ul .searchresfilter').each(function () {
        var datacount = 0;
        var datacount_all = 0;
        var obj = jQuery(this);
        var page_id = jQuery(this).data('pageid');
        if (page_id == 1) {
            jQuery('#search_fiter .searchdivs').each(function () {
                datacount_all++;
            });
            jQuery('.search_results_headline .total_search_count').html(datacount_all);
            if (datacount_all == 0) {
                obj.find("input[type='checkbox']").prop('checked', false);
                obj.find("input[type='checkbox']").prop('disabled', true);
            }
            if (datacount_all == 1) {
                jQuery("#hit_singular").removeClass("hide");
            } else {
                jQuery("#hit_plural").removeClass("hide");
            }
        } else {
            if (jQuery.type(page_id) === "string") {
                var pids = page_id.split(',');
                for (var i = 0; i < pids.length; i++) {
                    jQuery('#search_fiter .searchdivs').each(function () {
                        if (jQuery(this).data('rel1') == pids[i]) {
                            datacount++;
                        }
                    });
                }
                ...
                jQuery('#search_fiter .searchdivs').each(function () {
                    if (jQuery(this).data('rel1') == page_id) {
                        datacount++;
                    }
                });
            }
            obj.find('span.checkbox span').html(datacount);
            if (datacount == 0) {
                obj.find("input[type='checkbox']").prop('checked', false);
                obj.find("input[type='checkbox']").prop('disabled', true);
            }
    ...
    jQuery('.Headline_Copy_Process').slick({
        dots: true,
        arrows: false,
        slidesToShow: 3,
        slidesToScroll: 1,
        rows: 1,
        autoplay: true,
        nextArrow: '<div class="arrows next-arrow"><i class="material-icons">arrow_forward</i></div>',
        prevArrow: '<div class="arrows prev-arrow"><i class="material-icons">arrow_back</i></div>',
        touchThreshold: 100,
        responsive: [
            {
                breakpoint: 1380,
    ...
    jQuery('.main_home_slider').each(function () {
        var sliderAutoPlay = false;
        if (jQuery(this).hasClass('has-autoplay')) {
            sliderAutoPlay = true;
        }
    ...
    jQuery('.news-slider-wrap-var1 .news-slider').slick({
        dots: true,
        arrows: true,
        // autoplay:true,
        slidesToShow: 4,
    ...
    jQuery('.gallery_slider').each(function () {
        ...
        jQuery(this).slick({
            ...
            //adaptiveHeight: true,
            touchThreshold: 100,
            nextArrow: '<div class="gallery next-arrow"><i class="material-icons">arrow_forward</i></div>',
            prevArrow: '<div class="gallery prev-arrow"><i class="material-icons">arrow_back</i></div>'
        });
    });

    jQuery('.gallery_slider').on('afterChange', function (event, slick, currentSlide, nextSlide) {
        var text_content_height = jQuery(this).find('.slick-active .slider_text_content').innerHeight();
        var new_top = text_content_height / 2;
        jQuery(this).find('.gallery').css("top", "calc(50% - " + new_top + "px)");
    });

    var text_content_height = jQuery('.gallery_slider').find('.slick-active .slider_text_content').innerHeight();
    var new_top = text_content_height / 2;
    jQuery('.gallery_slider').find('.gallery').css("top", "calc(50% - " + new_top + "px)");


    jQuery('.product_boxes .news-slider').slick({
        dots: true,
        arrows: true,
        slidesToShow: 4,
        slidesToScroll: 1,
    ...
    function onNewsCheckboxClick(isCreateSlider, element) {
        width_variable = jQuery(window).width();
        var rowvar2array = new Array();
        var rowVar2 = "";
        jQuery('.news-slider-wrap-var2 .filter-chkbox ul li').each(function () {
            if (jQuery(this).find('input:checkbox').is(':checked')) {
                //alert(jQuery(this).data('category'));
                rowvar2array.push(jQuery(this).data('category'));
            }
        });
        if (rowvar2array.length) {
            //console.log(rowvar2array);
            var rowVar2 = "";
            var count = 0;
            ...
            var total = jQuery('#row-var2-content .col-sm-3').length;
            jQuery('#row-var2-content .col-sm-3').each(function () {
                var val = jQuery(this).data('category');
                if (jQuery.inArray(val, rowvar2array) !== -1) {
                    count++;
                    if (width_variable > 767) {
                        if (count === 9) {
                            rowVar2 += "</div>";
                    ...
                    rowVar2 += "<div class='col-sm-3'>";
                    rowVar2 += jQuery(this).html();
                    rowVar2 += "</div>";
            ...
            if (isCreateSlider) {
                jQuery(".news-slider-wrap-var2 .news-slider").slick('unslick');
            }
            jQuery('.news-slider-wrap-var2 .news-slider').html(rowVar2);
            if (isCreateSlider) {
                create_news_row_Var2_slider();
                create_news_row_Var2_slider_mobile();

                // manupulate browser's history
                if (rowvar2array.length == 3) {
                    pushState('');
                } else {
                    pushState('?filter=' + rowvar2array.join());
                }
            }
        } else {
    ...
    jQuery('.news-slider-wrap-var2 .filter-chkbox ul li input:checkbox').change(function () {
        onNewsCheckboxClick(true, this);
    });

    //filter search results
    jQuery('.search_results_headline .filter-chkbox ul li input:checkbox').change(function () {
        var rowvar2array = new Array();
        var rowVar2 = "";
        var showall = 0;
        var count = 0;
        jQuery('.search_results_headline .filter-chkbox ul li').each(function () {
            if (jQuery(this).find('input:checkbox').is(':checked')) {
                var pageid = jQuery(this).data('pageid');
                rowvar2array.push(pageid);
                //If 'inhalt' is selected, showall is set to '1' regardless of other filters
                if (pageid == '1') {
                    showall = 1;
                }
            }
        });
        //console.log("The page ids of filters selected are : " + rowvar2array);
        //If 'inhalt' is selected and other pages are also selected, show all search results
        if (rowvar2array.length && showall) {
            var rowVar2 = "";
            ...
            jQuery(".search-slider-wrap-var .search-slider-results").slick('unslick');
            jQuery('.search-slider-wrap-var .search-slider-results').html(rowVar2);
            create_search_row_Var2_slider(count);
        }
        //If filters other than 'inhalt' have been selected
        else if (rowvar2array.length) {
            var multipageindex; //Index to be split and then removed
            var strflag = 0 //Flag is set when a string exists in array rowvar2array
            for (i = 0; i < rowvar2array.length; i++) {
                //If there is a string in the array rowvar2array
                if (jQuery.type(rowvar2array[i]) === "string") {
                    var multipageid = rowvar2array[i].split(",");
                    multipageindex = i;
                    strflag = 1;
                }
            }
            if (strflag) {
                if (multipageindex > -1)
                    rowvar2array.splice(multipageindex, 1);

                //console.log("Removing the string element from rowvar2array");
                //console.dir(rowvar2array);

                for (var j = 0; j < multipageid.length; j++) {
                    rowvar2array.push(parseInt(multipageid[j]));
                }
                //console.log("Pushing the multi page ids into rowvar2array");
                //console.dir(rowvar2array);
            }
        ...
        //If nothing is selected, force the current filter the user is attempting to disable to be enabled
        else {
    ...
    jQuery('.banner_bottom_slider').slick({
        dots: false,
        arrows: true,
        autoplay: true,
        speed: 800,
    ...
    jQuery('.header_banner_slider').slick({
        dots: true,
        infinite: true,
        arrows: false,
        speed: 500,
        ...
        touchThreshold: 100,
        cssEase: 'linear',
        //appendArrows: jQuery(this).find(".slider_arrows")
    });


    jQuery('.slider_history .history_slider').on('init', function () {
        jQuery(".cboxslider .slick-current").prev().removeClass('nextslide').addClass('prevslide');
        jQuery(".cboxslider .slick-current").next().removeClass('prevslide').addClass('nextslide');
    });
    jQuery('.slider_history .history_slider').slick({
        dots: true,
        arrows: true,
        slidesToShow: 1,
        slidesToScroll: 1,
        ...
        nextArrow: '<div class="arrows next-arrow"><i class="material-icons">arrow_forward</i></div>',
        prevArrow: '<div class="arrows prev-arrow"><i class="material-icons">arrow_back</i></div>',
        //appendArrows: jQuery(".history_slider_arrow_div")
        touchThreshold: 100,
        responsive: [
            {
                breakpoint: 1380,
        ...
        jQuery(".cboxslider .slick-current").prev().removeClass('nextslide').addClass('prevslide');
        jQuery(".cboxslider .slick-current").next().removeClass('prevslide').addClass('nextslide');
        jQuery(".cboxslider .slick-current").removeClass('nextslide');
        jQuery(".cboxslider .slick-current").prev().removeClass('timepass');
        jQuery(".cboxslider .slick-current").next().removeClass('timepass');
        jQuery(".cboxslider .slick-current").removeClass('timepass');

    }).on('beforeChange', function () {
        jQuery(".cboxslider .slick-current").prev().addClass('timepass');
        jQuery(".cboxslider .slick-current").next().addClass('timepass');
    });

    jQuery('.slider_history .history_slider_mobile').slick({
        dots: true,
        arrows: false,
        slidesToShow: 1,
        slidesToScroll: 1,
    ...
    // jQuery('.header_right .language_btn').click(function () {
    //     jQuery(this).find('ul').toggleClass('bar_open');
    //     jQuery(this).toggleClass('lang_bar_open');
    // });

    jQuery('.header_right .back_btn').hover(function () {
        var width_var = jQuery(this).find('ul .icon').width() + 30;
        jQuery(this).find('ul').addClass('bar_open');
        jQuery(this).find('ul > li').css('min-width', width_var);
    }, function () {
        jQuery(this).find('ul').removeClass('bar_open');
        jQuery(this).find('ul > li').css('min-width', 70);
        //jQuery(this).toggleClass('lang_bar_open');
    });

    // jQuery('.header_right .contact_btn').click(function () {
    //     jQuery(this).toggleClass('bar_open');
    //     jQuery(this).find('.person_contact').toggleClass('lang_bar_open');
    // });

    jQuery(".header_right .contact_btn .kontakt_no a").click(function (event) {
        event.stopPropagation();
        // Do something
    });

    jQuery('.header_right .search_btn, .search_form_div .btn_custom').click(function () {
        jQuery('.search_div').toggleClass('hide_search');
        jQuery('.search_overlay').toggleClass('hide');
        jQuery("#tx-indexedsearch-searchbox-sword").focus();
    });


    //Navigation
    jQuery('.navbar-header').on('click', '.navbar-toggle.collapsed', function () {
        if (jQuery(window).width() < 1025) {
            jQuery('#myNavbar ul li').removeClass('open');
        }
    });

    jQuery('.navbar-header').on('click', '.navbar-toggle', function () {
        if (jQuery(window).width() < 1025) {
            setTimeout(function () {
                jQuery('.menu_list').scrollTop(0);
            }, 400);
        }
    });

    var bodyPosition = 0;
    jQuery("#myBtnNavbar").on('click', function () {
        if (jQuery(window).width() < 1025) {
            if (jQuery('html').hasClass('overflow')) {
                jQuery(this).addClass('collapsed');
                jQuery(this).attr('aria-expanded', 'false');
                $('body').removeAttr('style').scrollTop(bodyPosition);
            } else {
            ...
            jQuery('.main_overlay').toggleClass('hide');
        }
    });

    jQuery("#mainOverlay").on('click', function () {
        jQuery("#myBtnNavbar").trigger('click');
    });

    var width_variable = jQuery(window).width();
    jQuery(window).resize(function () {
        width_variable = jQuery(window).width();
        square_content_boxes(width_variable);
        headline_mobile_slider(width_variable);
        footer_menu_slider(width_variable);
        equal_height_function();

        if (jQuery(window).width() > 1024) {
            jQuery("html").removeClass('overflow');
            jQuery("#myBtnNavbar").attr('aria-expanded', 'false');
            jQuery("#myBtnNavbar").removeClass('collapsed');
            jQuery('.navbar-collapse').removeClass('in');
            jQuery('.main_overlay').addClass('hide');
    ...
    jQuery('#myNavbar ul li').click(function (event) {
        event.stopPropagation();
        jQuery(this).siblings().removeClass('open');
        jQuery(this).siblings().find('li').removeClass('open');
        if (jQuery(this).hasClass('open')) {
            jQuery(this).removeClass('open');
            jQuery(this).find('li').removeClass('open');
        } else {
    ...
    jQuery('#myNavbar ul.navbar-nav li.sub > a').click(function (event) {
        /*if (width_variable < 769) {*/
        if (width_variable < 1025) {
            event.preventDefault();
        }
    });


    //var add_list_padding = jQuery('.contact_boxes .filter-item .item-content .details_content_list').innerHeight();
    equal_height_contact_box_01();
    add_height_contact_box_01();
    equal_height_contact_box_02();


    var max_height = 0;
    jQuery('.boxes_tooltip .boxes_tooltip_item').each(function () {
        var check_height = jQuery(this).find('.boxes_tooltip_item_bg').innerHeight();
        if (check_height > max_height) {
            max_height = check_height;
        }
    });
    //console.log(max_height);
    jQuery('.boxes_tooltip .boxes_tooltip_item_bg').innerHeight(max_height);

    if (width_variable > 767) {
        jQuery('.boxes_tooltip .box_Tooltip_text_mobile').remove();
    }

    jQuery(".boxes_tooltip_item.tooltip_active").click(function () {

        if (width_variable > 767) {
            if (jQuery(this).hasClass('grey_dv')) {
                jQuery(this).removeClass('grey_dv');
            } else {
                jQuery('.boxes_tooltip_item').removeClass('grey_dv');
                jQuery(this).toggleClass("grey_dv");
            }

        } else {
            if (jQuery(this).hasClass('grey_dv')) {
                jQuery(this).removeClass('grey_dv');
                jQuery(this).parents('.row').find('.tool_tip_box_content').empty();
                jQuery(this).parents('.row').find('.box_Tooltip_text_mobile').removeClass('show');
            } else {
                jQuery('.boxes_tooltip_item').removeClass('grey_dv');
                jQuery('.tool_tip_box_content').empty();
                jQuery(this).toggleClass("grey_dv");
                jQuery(this).parents('.row').find('.box_Tooltip_text_mobile').addClass('show');
                var content = jQuery(this).find(".tooltiptext_boxes").html();
                var boxnum = jQuery(this).parents('.box_iteration').data('boxnum');
                var res = boxnum.split("_");
                var numb = parseInt(res[1]);
                //var check_even = isEven(2);
                jQuery(this).parents('.row').find('.tool_tip_box_content.' + boxnum).html(content);
            }
        }
    });
    ...
    var active_text = jQuery('.header_right .language_btn ul li.active span').text();
    //console.log('span-> ' + active_text);
    jQuery('.header_right .language_btn a .language_icon span').text(active_text);


    jQuery('.video_play_btn').click(function () {
        var youtube_link = jQuery(this).data('videolink');
        var object = jQuery(this);
        //createVimeoEmbed('https://vimeo.com/222545163', 'banner_bottom_video');
        createYouTubeEmbed(youtube_link, object, 'banner_bottom_video');
        jQuery(this).hide();
        jQuery(this).parents('.post_video').find(".banner_bottom_video iframe")[0].src += "&autoplay=1";
        jQuery(this).parents('.post_video').find('.overlayvideo_thumbnail').hide();
        /*jQuery(".banner_bottom_video iframe")[0].src += "&autoplay=1";
         jQuery('.overlayvideo_thumbnail').hide();*/
    });

    /* OLD filter 01
     //Creation of the JSON object for filtering in Contact Boxes 01 Module
     if (jQuery('#filter_This').data('formkey')) {
     var filter_Val_with_brack = jQuery('#filter_This').data('formkey');
     var filter_Val_with_curl = filter_Val_with_brack.replace(/\[/g, '{').replace(/]/g, '}');
     //alert(filter_Val_with_curl);

     //var filter_Valbtoa = btoa(filter_Val_with_curl);
     var filter_Valbtoa = btoa(unescape(encodeURIComponent(filter_Val_with_curl)));

     //jQuery('#filter_This').attr('data-formkey', ''); //empty value and store in localstorage
     localStorage.setItem("filter_This", filter_Valbtoa);
     var filter_Valbtoa = localStorage.getItem("filter_This");
     var filter_Valatob = atob(filter_Valbtoa);
     var jsonObj = JSON.parse('[' + filter_Valatob + ']');
     //console.log(jsonObj);
    ...
    //Creation of the JSON object for filtering in Contact Boxes 02 Module
    if (jQuery('#filter_This_two').data('formkey')) {
        var filter_Val_with_brack_two = jQuery('#filter_This_two').data('formkey');
        var filter_Val_with_curl_two = filter_Val_with_brack_two.replace(/\[/g, '{').replace(/]/g, '}');

        var filter_Valbtoa_two = btoa(unescape(encodeURIComponent(filter_Val_with_curl_two)));
        jQuery('#filter_This_two').attr('data-formkey', ''); //empty value and store in localstorage
        localStorage.setItem("filter_This_two", filter_Valbtoa_two);
        var filter_Valbtoa_two = localStorage.getItem("filter_This_two");
        var filter_Valatob_two = decodeURIComponent(escape(atob(filter_Valbtoa_two)));
        var jsonObj_two = JSON.parse('[' + filter_Valatob_two + ']');
    }

    //Fix the dots overlapping the contact boxes
    jQuery('.contact_boxes').find('.slick-dots').each(function () {
        var dots_height = jQuery(this).css("height");
        if (dots_height != '0px') {
            var slider_parent = jQuery(this).closest('.slider-wrap-mobile');
            slider_parent.css("padding-bottom", dots_height);
        }
    });

    //Fix the dots overlapping for news slider variation 1 (on mobile)
    jQuery('.news-slider-wrap-var1').find('.slick-dots').each(function () {
        var dots_height = jQuery(this).css("height");
        if (dots_height != '0px') {
            var slider_parent = jQuery(this).closest('.mobile_version');
            slider_parent.css("padding-bottom", dots_height);
        }
    });

    //Fix the dots overlapping for news slider variation 2 (on mobile)
    jQuery('.news-slider-wrap-var2').find('.slick-dots').each(function () {
        var dots_height = jQuery(this).css("height");
        if (dots_height != '0px') {
            var slider_parent = jQuery(this).closest('.mobile_version');
            slider_parent.css("padding-bottom", dots_height);
    ...
    //Fix the dots overlapping for news slider variation 1 (on desktop)
    jQuery('.news-slider-wrap-var1.desktop_version').find('.slick-dots').each(function () {
        console.log("Found variation 1");
        var dots_height = jQuery(this).css("height");
        if (dots_height != '0px') {
            var slider_parent = jQuery(this).closest('.desktop_version');
            slider_parent.css("padding-bottom", dots_height);
        }
    });

    //Fix the dots overlapping for news slider variation 2 (on desktop)
    jQuery('.news-slider-wrap-var2.desktop_version').find('.slick-dots').each(function () {
        console.log("Found variation 2");
        var dots_height = jQuery(this).css("height");
        if (dots_height != '0px') {
            var slider_parent = jQuery(this).closest('.desktop_version');
            slider_parent.css("padding-bottom", dots_height);
    ...
    /* OLD filter 01
     jQuery("#filter_ul_cb li").click(function (e) {
     /*.e event
     e.preventDefault();
     var rowSize = jQuery('.slider-wrap').attr('data-rowSize');

     var filterli = jQuery(this).data('filterli');

     if (jQuery(this).find("input[name='selector']").is(':checked')) {
     var returnedData = jsonObj;
     jQuery(this).find("input[name='selector']").prop('checked', false);
     } else {
     var returnedData = jQuery.grep(jsonObj, function (n, i) {
     return n.country_selector === filterli;
     });
     jQuery(this).find("input[name='selector']").prop('checked', true);
     }

     /*var returnedData = $.grep(jsonObj, function (n, i) {
     return n.country_selector === filterli;
     });

     //var returnedData = jsonObj;

     // Create an html markup with received data
     var html_content_normal = "<div class='rows slider'>";
     var html_content_responsive = "";
     for (var i = 0; i < returnedData.length; i++) {
     //normal
     html_content_normal +=
     "<div class='col-sm-3 col-xs-6 slider_no_padding'>" +
     "<div class='filter-item'>" +
     "<div class='item-content'>";
     if (returnedData[i].Headline != "")
     html_content_normal += "<div class='position'><p>" + returnedData[i].Headline + "</p></div>";
     //if (returnedData[i].Address != "")
     html_content_normal += "<address class='address'>" + returnedData[i].Address + "</address>";
     //      opening of wrapper
     html_content_normal += "<div class='details_content_list'>";
     if (returnedData[i].Tel != "")
     html_content_normal += "<div class='details_info'><a href='tel:" + returnedData[i].Tellink + "'>Tel.: " + returnedData[i].Tel + "</a></div>";
     if (returnedData[i].Fax != "")
     html_content_normal += "<div class='details_info'><a href='fax:" + returnedData[i].Faxlink + "'>Fax: " + returnedData[i].Fax + "</a></div>";
     if (returnedData[i].Email != "")
     html_content_normal += "<div class='details_info'><a href='mailto:" + returnedData[i].Email + "'>E-Mail: " + returnedData[i].Email + "</a></div>";
     if (returnedData[i].Web != "")
     html_content_normal += "<div class='details_info'><a href='http://" + returnedData[i].Web + "' target='_blank'>Web: " + returnedData[i].Web + "</a></div>";
     if (returnedData[i].Mobile != "")
     html_content_normal += "<div class='details_info'><a href='tel:" + returnedData[i].Mobilelink + "'>Tel.: " + returnedData[i].Mobile + "</a></div>";
     //      closing of wrapper
     // alert("Tel: "+returnedData[i].Tel+" Fax: "+returnedData[i].Fax+" Email: "+returnedData[i].Email+" Web: "+returnedData[i].Web+" Mobile: "+returnedData[i].Mobilelink);
     html_content_normal += "</div>";
     html_content_normal +=
     "</div>" +
     "</div>" +
     "</div>";
     if (((i + 1) % 4) == 0) //&& ( (i+1) != returnedData.length )
     html_content_normal += "</div>";

     if ((((i + 1) % 4) == 0) && ((i + 1) != returnedData.length))
     html_content_normal += "<div class='rows slider'>";

     //responsive
     html_content_responsive +=
     "<div class='col-sm-3 col-xs-6 slider_no_padding no_padding_left'>" +
     "<div class='filter-item'>" +
     "<div class='item-content'>";
     if (returnedData[i].Headline != "")
     html_content_responsive += "<div class='position'>" + returnedData[i].Headline + "</div>";
     // if (returnedData[i].Address != "")
     html_content_responsive += "<address class='address'>" + returnedData[i].Address + "</address>";
     //      opening of wrapper
     html_content_responsive += "<div class='details_content_list'>";
     if (returnedData[i].Tel != "")
     html_content_responsive += "<div class='details_info'><a href='tel:" + returnedData[i].Tellink + "'>Tel.:" + returnedData[i].Tel + "</a></div>";
     if (returnedData[i].Fax != "")
     html_content_responsive += "<div class='details_info'><a href='fax:" + returnedData[i].Faxlink + "'>Fax: " + returnedData[i].Fax + "</a></div>";
     if (returnedData[i].Email != "")
     html_content_responsive += "<div class='details_info'><a href='mailto:" + returnedData[i].Email + "'>E-mail: " + returnedData[i].Email + "</a></div>";
     if (returnedData[i].Web != "")
     html_content_responsive += "<div class='details_info'><a href='http://" + returnedData[i].Web + "' target='_blank'>Web: " + returnedData[i].Web + "</a></div>";
     if (returnedData[i].Mobile != "")
     html_content_responsive += "<div class='details_info'><a href='tel:" + returnedData[i].Mobilelink + "'>Tel: " + returnedData[i].Mobile + "</a></div>";
     //      closing of wrapper
     html_content_responsive += "</div>";
     html_content_responsive +=
     "</div>" +
     "</div>" +
     "</div>";
     //if (((i + 1) % 2) == 0) //&& ( (i+1) != returnedData.length )
     // html_content_responsive += "</div><div class='row1 slider'>";
     }
     html_content_normal += "</div>";
     html_content_responsive += "";


     //console.log(width_variable+' ====');
     jQuery(".contact_boxes .slider-wrap").slick('unslick');
     jQuery(".contact_boxes .slider-wrap").html(html_content_normal);

     jQuery(".slider-wrap-mobile .slider").slick('unslick');
     jQuery(".slider-wrap-mobile .slider").html(html_content_responsive);
    ...
    //Contact Boxes 02 Module filtering function
    jQuery("#filter_ul_cb_02 li").click(function (e) {
        /*.e event */
        e.preventDefault();
        var filterli = jQuery(this).data('filterli'); //Get country to be filtered


        if (jQuery(this).find("input[name='selector']").is(':checked')) {
            var returnedData = jsonObj_two;
            jQuery(this).find("input[name='selector']").prop('checked', false);
        } else {
            var returnedData = jQuery.grep(jsonObj_two, function (n, i) {
                return n.country_selector === filterli;
            });
            jQuery(this).find("input[name='selector']").prop('checked', true);
        }
        ...
        /*
         var returnedData = $.grep(jsonObj_02, function (n, i) {
         return n.country_selector === filterli;
         });
         */

        /* Create an html markup with received data */
        var html_content_normal = "<div class='row slider'>";
        for (var i = 0; i < returnedData.length; i++) {
            //normal
            html_content_normal +=
                "<div class='col-sm-3'>" +
                ...
                "<div class='item-content'>";
            if (returnedData[i].Headline != "")
                html_content_normal += "<div class='position'>" + returnedData[i].Headline + "</div>";
            if (returnedData[i].Address != "")
                html_content_normal += "<address class='address'>" + returnedData[i].Address + "</address>";
            //      opening of wrapper
            html_content_normal += "<div class='details_content_list'>";
            if (returnedData[i].Tel != "")
                html_content_normal += "<div class='details_info'><a href='tel:" + returnedData[i].Tellink + "'>" + returnedData[i].Tel + "</a></div>";
            if (returnedData[i].Fax != "")
                html_content_normal += "<div class='details_info'><a href='fax:" + returnedData[i].Faxlink + "'>" + returnedData[i].Fax + "</a></div>";
            if (returnedData[i].Email != "")
                html_content_normal += "<div class='details_info'><a href='mailto:" + returnedData[i].Email + "'>" + returnedData[i].Email + "</a></div>";
            if (returnedData[i].Web != "")
                html_content_normal += "<div class='details_info'><a href='http://" + returnedData[i].Web + "' target='_blank'>" + returnedData[i].Web + "</a></div>";
            if (returnedData[i].Mobile != "")
                html_content_normal += "<div class='details_info'><a href='tel:" + returnedData[i].Mobilelink + "'>" + returnedData[i].Mobile + "</a></div>";
            //      closing of wrapper
            html_content_normal += "</div>";
            html_content_normal +=
                "</div>" +
                ...
                "</div>";
            //if (((i + 1) % 4) == 0) //&& ( (i+1) != returnedData.length )
            //  html_content_normal += "</div><div class='row slider'>";

            if (((i + 1) % 4) == 0) //&& ( (i+1) != returnedData.length )
                html_content_normal += "</div>";

            if ((((i + 1) % 4) == 0) && ((i + 1) != returnedData.length))
                html_content_normal += "<div class='row slider'>";
        }
        html_content_normal += "</div>";

        jQuery(".contact_boxes .slider-wrap1").html(html_content_normal);

        equal_height_contact_box_02();
    });


    jQuery("#search_container_mobile_input").on('change', function (e) {
        search_mobile_transfer(jQuery(this));
    });
    jQuery("#search_container_mobile_submit").on('click', function (e) {
        search_mobile_submit_form();
    });
    jQuery("#search_container_mobile_input").on('keyup', function (e) {
        if (e.keyCode == 13) {
            search_mobile_submit_form();
        }
    });
    jQuery("#search_container_mobile_input").on('paste', function (e) {
        if (e.keyCode == 13) {
            search_mobile_submit_form();
        }
    });

    if (jQuery(".search_results_headline span#searchword_span").length) {
        var text = jQuery(".search_results_headline span#searchword_span").html();
        jQuery(".search_results_headline span#searchword_span").html(trim11(text));
        //console.log("text is-->" + trim11(text));
    }

    var searchForm = jQuery('#tx_indexedsearch');

    $(document)
        .on('click', '.tx-indexedsearch-autocomplete-words li', function () {
    ...
    jQuery('#filter_ul_cb li').click(function (e) {
        e.preventDefault();
        var htmlreplace = '';
        htmlreplace += '<div class="mydiv">';
        var htmlreplace_mobile = '';
        var country = jQuery(this).find('input').val();
        var myLength = jQuery(this).closest(".contact_boxes").find('.contactboxesfil .col-sm-3[data-filter="' + country + '"]').length;
        var count = 0;
        var filteroption = 'filter';
        if (jQuery(this).find("input").is(':checked')) {
            country = "1";
            filteroption = 'show';
            myLength = jQuery(this).closest(".contact_boxes").find('.contactboxesfil .col-sm-3[data-show="1"]').length;
            jQuery(this).find("input").prop('checked', false);
        } else {
            jQuery(this).find("input").prop('checked', true);
        }

        jQuery(this).closest(".contact_boxes").find('.contactboxesfil .col-sm-3').each(function (index) {
            if (jQuery(this).data(filteroption) == country) {
                //index = index + 1;
                count = count + 1;
                htmlreplace += "<div class='col-sm-3 col-xs-6 slider_no_padding'>";
                htmlreplace += jQuery(this).html();
                htmlreplace += '</div>';
                if (count % 4 == 0 && (count == myLength)) //if 4th box or last box
                    htmlreplace += '</div>';
                else if (count % 4 == 0 && (count != 0))
                    htmlreplace += '</div><div class="mydiv " data-mylength=' + myLength + ' data-countt=' + count + '>';
                htmlreplace_mobile += "<div class='col-sm-3 col-xs-6 slider_no_padding'>";
                htmlreplace_mobile += jQuery(this).html();
                htmlreplace_mobile += '</div>';
            }
        });
        jQuery(this).closest(".contact_boxes").find(".slider-wrap").slick('unslick');
        jQuery(this).closest(".contact_boxes").find(".slider-wrap").html(htmlreplace);

        jQuery(this).closest(".contact_boxes").find(".slider-wrap-mobile .slider").slick('unslick');
        jQuery(this).closest(".contact_boxes").find(".slider-wrap-mobile .slider").html(htmlreplace_mobile);


        create_slider_wrap_slider(jQuery(this).closest(".contact_boxes").find(".slider-wrap"));
        create_slider_wrap_slider_mobile();
//        equal_height_contact_box_01();
    });
});
...
jQuery(window).load(function () {
    setTimeout(function () {
        var dots_height = jQuery('.search-slider-wrap-var').find('.slick-dots').innerHeight();
        jQuery('.search-slider-wrap-var').css('padding-bottom', dots_height);
    }, 1000);
...
jQuery(".language_changer").click(function (event) {
    event.preventDefault();
    var linkit = jQuery(this).data("linkit");
    var form = $('#tx_indexedsearch');
    form.attr('action', linkit);
    form.submit();
});

jQuery(document).click(function (e) {
    var container = jQuery('.boxes_tooltip_item');
...
jQuery(document).ready(function () {
    jQuery('.tooltipn').tooltipster({
        ...
        functionAfter: function (instance, helper) {
            $(helper.origin).css('fill-opacity', '1');
        }
    });

    jQuery('.basetip').tooltipster({
        delay: 0,
        theme: 'tooltipster-shadow tooltipster-base-customized',
        'maxWidth': 270, // set max width of tooltip box
        contentAsHTML: true, // set title content to html
        interactive: true,
        trigger: 'custom', // add custom trigger
    ...
    jQuery(".basetip").mouseenter(function () {
        jQuery(this).css("fill-opacity", ".7");
    });
    jQuery(".basetip").mouseleave(function () {
        jQuery(this).css("fill-opacity", "1");
    });


    jQuery(".tooltipn").click(function (event) {
        event.preventDefault();
        if (jQuery(this).css("fill-opacity") == "0") {
            jQuery(this).css("fill-opacity", "1");
        } else {
            jQuery(".tooltipn").each(function () {
                jQuery(this).css('fill-opacity', '1');
            });
            jQuery(this).css("fill-opacity", "0");
        }
    });

    if (jQuery(".contact_header_module").length) {
        jQuery(".person_contact").html(
            jQuery(".contact_header_module").html()
        );
        //console.log("done");
    } else {
        //console.log("checked");
...
var cookies = function () {
    var $el = jQuery('#cookies');
    if (localStorage.cookies === undefined) {
        $el.addClass('active');
        $el.find('.btn_c').click(function () {
            try {
    ...
    jQuery('table.contenttable').each(function () {
        if ($("th", this).length > 0) {
            ...
            $table.children('thead').children().children().each(function (i) {
                var text = $(this).text();
                $rows.each(function () {
                    $(this).children().eq(i).attr('data-th', text);
                });
            });
        }
    });
...
function myMap() {
    ...
    var contentString1 = '<div id="content">' +
        '<div id="siteNotice">' +
        '</div>' +
        '<h5 id="firstHeading" class="black">KNOLL Maschinenbau GmbH</h5>' +
        '<div id="bodyContent">' +
        '<div>Test 1 dsdds </div>' +
        '</div>' +
        '</div>';
...
function createVimeoEmbed(url, container) {
    ...
    var vimeopreview = "<div class='vimeoFrame'><iframe id='player_" + section_id + "' class='video_container embed-responsive-item vimeoXtra' width='100%' height='535' src='" + protocol + "://player.vimeo.com/video/" + vimeoID + "?autoplay=1&player_id=player'  frameborder='0' webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>";
...
function createYouTubeEmbed(url, object, container) {
    var section_id = 1;
    var youtubeURL = url;
    ...
    var youtubepreview = "<div class='vimeoFrame'><iframe id='player_" + section_id + "' class='video_container embed-responsive-item' width='100%' height='535' src='https://www.youtube.com/embed/" + youtubeID + "?rel=0&controls=1&showinfo=0&modestbranding=1' frameborder='0' allowfullscreen allowfullscreen='true' webkitallowfullscreen='true' mozallowfullscreen='true'></iframe></div>";
    //jQuery("." + container).prepend(youtubepreview);
    object.parents('.post_video').find("." + container).prepend(youtubepreview);
}

function headline_mobile_slider(width_variable) {
    var width = width_variable;
    if (width < 768) {
        jQuery('#headline_mobile_slider').addClass('Headline_Copy_Process');
        jQuery('.Headline_Copy_Process').slick(getSliderSettings());
    } else {
        jQuery('#headline_mobile_slider').removeClass('Headline_Copy_Process');
    }
}
...
function getSliderSettingsFooter() {
    return {
        dots: false,
        arrows: true,
        infinite: false,
...
function footer_menu_slider(width_variable) {
    var width = width_variable;
}

function create_slider_wrap_slider($el) {
    var rowSize = jQuery('.slider-wrap').attr('data-rowSize');
    jQuery('.slider_wrap').on('init', function () {
        jQuery(".slider-wrap.desktop .slick-current").prev().removeClass('nextslide').addClass('prevslide');
        jQuery(".slider-wrap.desktop .slick-current").next().removeClass('prevslide').addClass('nextslide');
    });

    $el = $el === undefined ? jQuery('.slider-wrap') : $el;
    ...
    $el.slick({
        ...
        jQuery(".slider-wrap.desktop .slick-current").prev().removeClass('nextslide').addClass('prevslide');
        jQuery(".slider-wrap.desktop .slick-current").next().removeClass('prevslide').addClass('nextslide');
        jQuery(".slider-wrap.desktop .slick-current").removeClass('nextslide');
    });
}

function create_slider_wrap_slider_mobile() {
    jQuery('.slider-wrap-mobile .slider').slick({
        dots: true,
        arrows: false,
        slidesToShow: 2,
        slidesToScroll: 2,
...
function create_news_row_Var2_slider() {
    jQuery('.news-slider-wrap-var2 .news-slider.desktop_version').slick({
        dots: true,
        arrows: true,
        // autoplay:true,
        //slidesToShow: 4,
...
function create_news_row_Var2_slider_mobile() {
    jQuery('.news-slider-wrap-var2 .news-slider.mobile_version').slick({
        dots: true,
        arrows: false,
        slidesToShow: 2,
        slidesToScroll: 2,
...
function create_search_row_Var2_slider(count) {
    var rows = '';
    if (count > 4) {
        rows = 2;
    } else {
    ...
    jQuery('.search-slider-wrap-var .search-slider-results').slick({
        dots: true,
        arrows: true,
        slidesToShow: 3,
        slidesToScroll: 3,
...
function add_height_contact_box_01() {
    //var add_list_padding = 0;
    jQuery('.contact_boxes .slick-slider .filter-item .item-content').each(function () {
        var check_height = jQuery(this).find('.details_content_list').innerHeight();
        /*if (check_height > add_list_padding) {
         add_list_padding = check_height;
         }*/
        jQuery('.contact_boxes .contactboxesfil .filter-item .item-content').css('padding-bottom', check_height);
    });
    //jQuery('.contact_boxes .slick-slider .filter-item .item-content').css('padding-bottom', add_list_padding);


    setTimeout(function () {
        var max_height = 0;
        jQuery('.contact_boxes .slick-slider .filter-item .item-content').each(function () {
            var check_height = jQuery(this).innerHeight();
            if (check_height > max_height) {
                max_height = check_height;
            }
        });
        jQuery('.contact_boxes .contactboxesfil .filter-item .item-content').innerHeight(max_height);
    }, 500);
}

function equal_height_contact_box_01() {
    //var add_list_padding = 0;
    jQuery('.contact_boxes .slick-slider .filter-item .item-content').each(function () {
        var check_height = jQuery(this).find('.details_content_list').innerHeight();
        /*if (check_height > add_list_padding) {
        ...
        jQuery(this).css('padding-bottom', check_height);
    });
    //jQuery('.contact_boxes .slick-slider .filter-item .item-content').css('padding-bottom', add_list_padding);
    ...
    setTimeout(function () {
        ...
        jQuery('.contact_boxes .slick-slider .filter-item .item-content').innerHeight(max_height);
    }, 500);
//    console.log("equal_height_contact_box_01 applied");
}

function equal_height_contact_box_02() {
    var add_list_padding = 0;
    jQuery('.contact_boxes_02 .filter-item .item-content').each(function () {
        var check_height = jQuery(this).find('.details_content_list').innerHeight();
        /*if (check_height > add_list_padding) {
         add_list_padding = check_height;
         }*/
    ...
    //jQuery('.contact_boxes_02 .filter-item .item-content').css('padding-bottom', add_list_padding);
    ...
    setTimeout(function () {
        ...
        jQuery('.contact_boxes_02 .filter-item .item-content').innerHeight(max_height);
    }, 500);
}

function search_mobile_transfer(thisobj) {
    jQuery("#tx-indexedsearch-searchbox-sword").val(thisobj.val());
}

function search_mobile_submit_form() {
    jQuery("#tx_indexedsearch").submit();
}

//if (jQuery("div.news.news-single").length) {
//    jQuery(".language_btn ul li").each(function () {
//        var thisobj = jQuery(this);
//        var hreff = jQuery(this).find("a").attr('href');
//        //console.log(hreff);
//        $.get(hreff, function (data) {
//            //console.log( data );
//            if (data.indexOf("news-single") >= 0) {
//                //console.log(hreff + " there");
//                //thisobj.css("background","green");
//            } else {
//                //console.log(hreff + " none");
...
function equal_height_function() {

    jQuery('.contact_boxes_02').each(function () {
        var max_height = 0;

        jQuery(this).find('.filter-item .position').each(function () {
            var height = jQuery(this).height();
            if (height > max_height) {
                max_height = height;
            }
        });
        jQuery(this).find('.filter-item .position').height(max_height);
    });
    ...
    jQuery('.contact_boxes_02').each(function () {
        ...
        jQuery(this).find('.filter-item .item-content').each(function () {
            var height = jQuery(this).innerHeight();
            if (height > max_height) {
                max_height = height;
            }
        });
        jQuery(this).find('.filter-item .item-content').innerHeight(max_height);
    });

    jQuery('.history_slider').each(function () {
        var max_height = 0;

        jQuery(this).find('.text_container').each(function () {
            var height = jQuery(this).innerHeight();
            if (height > max_height) {
                max_height = height;
            }
        });
        jQuery(this).find('.text_container').innerHeight(max_height);
    });

    jQuery('.history_slider_mobile').each(function () {
        var max_height = 0;
    ...
    jQuery('.news-slider-wrap').each(function () {
        var max_height = 0;

        jQuery(this).find('.post-item .post_content').each(function () {
            var height = jQuery(this).innerHeight();
            if (height > max_height) {
                max_height = height;
            }
        });
        jQuery(this).find('.post-item .post_content').innerHeight(max_height);
    });

    jQuery('.downloads-boxes').each(function () {
        var max_height = 0;

        jQuery(this).find('.downloads-item .download-wrap').each(function () {
            var height = jQuery(this).innerHeight();
            if (height > max_height) {
                max_height = height;
            }
        });
        jQuery(this).find('.downloads-item .download-wrap').innerHeight(max_height);
    });


    jQuery('.history_no_slider').each(function () {
        var max_height = 0;
        jQuery(this).find('.text_container').each(function () {
            var height = jQuery(this).innerHeight();
            if (height > max_height) {
    ...
    setTimeout(function () {
        jQuery('.main_home_slider').each(function () {
            ...
            jQuery(this).find('.slick-slide').each(function () {
                var height = jQuery(this).find('.banner_bottom > .container').outerHeight();
                if (height > max_height) {
                    max_height = height;
                }
            });
            jQuery(this).find('.slick-slide .banner_bottom').height(max_height);
        });

        var marginBottom = jQuery('.main_home_slider .slick-slide .banner_bottom').innerHeight() + 20;
        //var arrow_bottom = jQuery('.main_home_slider .slick-slide .banner_bottom').innerHeight() - jQuery('.main_home_slider .prev-arrow').innerHeight() - 10;
        var arrow_bottom = jQuery('.main_home_slider .slick-slide .banner_bottom').innerHeight() - jQuery('.main_home_slider .prev-arrow').innerHeight() - 20;
        if (arrow_bottom < 0) {
            arrow_bottom = 0;
        }

        jQuery('.main_home_slider .slick-dots').css('margin-bottom', marginBottom);
        jQuery('.main_home_slider .slick-arrow').css('bottom', arrow_bottom);
    }, 100);
...
var anchor = function () {
    ...
    anchor.prototype.scroll = function (target) {
        if (target.length === 0) return false;

        jQuery('html, body').animate({
            scrollTop: target.offset().top - jQuery('body > div.header_div > nav').position().top - jQuery('body > div.header_div > nav').outerHeight()
        }, 1000);
    };

    $document.on('click', 'a[href*="#"]', function (e) {
        if (jQuery(this).attr('href').match(/#\w+/)) {
            ...
            if (location.hostname === this.hostname) {
                ...
                if (target.length) {
                    e.preventDefault();
                    anchor.prototype.scroll(target);
                }
            }
        }
    });
...
function lazyload() {
    ...
    ll.picture.not('[data-lazyload="false"]').each(function () {
        $(this).children('img')
            .attr('src', 'data:image/gif;base64,R0lGODlhAQABAAAAADs=')
            .addClass('lazyload');
    });

    $('[data-lazy]').addClass('lazyload');
}
...
Hyphenator = (function (window) {
    ...
    function css3_gethsupport() {
        var support = false, supportedBrowserLangs = {}, property = "", checkLangSupport,
        ...
        var factory = function () {
            ...
            var oncopyHandler = function (e) {
                ...
                var shadow, selection, range, rangeShadow, restore, target = e.target || e.srcElement,
                    currDoc = target.ownerDocument, bdy = currDoc.getElementsByTagName("body")[0],
    ...
    function autoSetMainLanguage(w) {
        w = w || contextWindow;
        var el = w.document.getElementsByTagName("html")[0], m = w.document.getElementsByTagName("meta"), i = 0,
    ...
    function convertPatternsToArray(lo) {
        var trieNextEmptyRow = 0, i, charMapc2i, valueStore, indexedTrie, trieRowLength,
    ...
    function hyphenateWord(lo, lang, word) {
        var pattern = "", ww, wwlen, wwhp = wwhpStore, pstart = 0, plen, hp, hpc, wordLength = word.length, hw = "",
            charMap = lo.charMap.code2int, charCode, mappedCharCode, row = 0, link = 0, value = 0, values,
            indexedTrie = lo.indexedTrie, valueStore = lo.valueStore.keys, wwAsMappedCharCode = wwAsMappedCharCodeStore;
    ...
    function getConfigFromURI() {
        var loc = null, re = {}, jsArray = contextWindow.document.getElementsByTagName("script"), i = 0, j = 0,
            l = jsArray.length, s, gp, option;
        while (i < l) {
...
Hyphenator.config({
    selectorfunction: function selectorFunction() {
        if (document.documentElement.lang === 'de') {
            return window.document.querySelectorAll('.hyphenate,h1,h2,h3,h4');
        } else {
            return false;
        }
    }, minwordlength: 11
...
(function () {
    ...
    google.maps.Load = function (apiLoad) {
        ...
        apiLoad([0.009999999776482582, [null, [["https://khms0.googleapis.com/kh?v=851\u0026hl=en-US\u0026", "https://khms1.googleapis.com/kh?v=851\u0026hl=en-US\u0026"], null, null, null, 1, "851", ["https://khms0.google.com/kh?v=851\u0026hl=en-US\u0026", "https://khms1.google.com/kh?v=851\u0026hl=en-US\u0026"]], null, null, null, null, [["https://cbks0.googleapis.com/cbk?", "https://cbks1.googleapis.com/cbk?"]], [["https://khms0.googleapis.com/kh?v=125\u0026hl=en-US\u0026", "https://khms1.googleapis.com/kh?v=125\u0026hl=en-US\u0026"], null, null, null, null, "125", ["https://khms0.google.com/kh?v=125\u0026hl=en-US\u0026", "https://khms1.google.com/kh?v=125\u0026hl=en-US\u0026"]], [["https://mts0.googleapis.com/mapslt?hl=en-US\u0026", "https://mts1.googleapis.com/mapslt?hl=en-US\u0026"]], null, null, null, [["https://mts0.googleapis.com/mapslt?hl=en-US\u0026", "https://mts1.googleapis.com/mapslt?hl=en-US\u0026"]]], ["en-US", "US", null, 0, null, null, "https://maps.gstatic.com/mapfiles/", null, "https://maps.googleapis.com", "https://maps.googleapis.com", null, "https://maps.google.com", null, "https://maps.gstatic.com/maps-api-v3/api/images/", "https://www.google.com/maps", 0, "https://www.google.com"], ["https://maps.googleapis.com/maps-api-v3/api/js/37/10a", "3.37.10a"], [3870609670], null, null, null, null, null, null, "myMap", null, null, 1, "https://khms.googleapis.com/mz?v=851\u0026", "AIzaSyAFwS3O6xARMaRBg8CvBrKcBBNQVf2txVc", "https://earthbuilder.googleapis.com", "https://earthbuilder.googleapis.com", null, "https://mts.googleapis.com/maps/vt/icon", [["https://maps.googleapis.com/maps/vt"], ["https://maps.googleapis.com/maps/vt"], null, null, null, null, null, null, null, null, null, null, ["https://www.google.com/maps/vt"], "/maps/vt", 478000000, 478, 478187519], 2, 500, [null, null, null, null, "https://www.google.com/maps/preview/log204", "", "https://static.panoramio.com.storage.googleapis.com/photos/", ["https://geo0.ggpht.com/cbk", "https://geo1.ggpht.com/cbk", "https://geo2.ggpht.com/cbk", "https://geo3.ggpht.com/cbk"], "https://maps.googleapis.com/maps/api/js/GeoPhotoService.GetMetadata", "https://maps.googleapis.com/maps/api/js/GeoPhotoService.SingleImageSearch", ["https://lh3.ggpht.com/", "https://lh4.ggpht.com/", "https://lh5.ggpht.com/", "https://lh6.ggpht.com/"]], null, null, null, null, "/maps/api/js/ApplicationService.GetEntityDetails", 0, null, null, null, null, [], ["37.10a"], 1, 0, [1]], loadScriptTime);
    };
    var loadScriptTime = (new Date).getTime();
})();
// inlined
(function (_) {
    var ua, va, za, Aa, Ea, Fa, Ga, Ha, Ia, Za, ab, vb, Lb, Mb, Nb, Pb, Qb, Tb, Ub, Wb, Xb, Yb, $b, dc, lc, sc, rc, Ac,
        Fc, Gc, Ic, Uc, Wc, $c, hd, jd, kd, od, wd, Ad, Bd, Fd, Od, Pd, Qd, Rd, Td, Ud, Xd, $d, Wd, de, ie, te, ue, ye,
        Ae, Ce, De, Be, Fe, Ie, Ke, Le, Ee, He, Je, Me, Pe, Qe, Re, hf, jf, kf, mf, nf, pf, qf, uf, vf, wf, xf, yf, Af,
        Df, Ef, Mf, Nf, Of, Qf, Vf, Yf, dg, $f, hg, gg, bg, Wf, Tf, ng, og, pg, wg, zg, Ag, Eg, Fg, Gg, Hg, Ig, Jg, Kg,
        Qg, Rg, Tg, Sg, Zg, Ug, ah, Xg, Yg, hh, eh, ih, jh, lh, ph, rh, qh, th, xh, Ah, zh, Dh, Eh, Fh, Ih, Jh, Th, Sh,
        Kh, Lh, ya, xa, Xh, Ma, La, Wa, Xa;
    _.aa = "ERROR";
    ...
    _.z = function (a, b, c) {
        Function.prototype.bind && -1 != Function.prototype.bind.toString().indexOf("native code") ? _.z = Za : _.z = ab;
    ...
    _.C = function (a, b, c) {
        for (var d = a.length, e = _.Ja(a) ? a.split("") : a, f = 0; f < d; f++) f in e && b.call(c, e[f], f, a)
    };
    _.eb = function (a, b) {
        for (var c = a.length, d = [], e = 0, f = _.Ja(a) ? a.split("") : a, g = 0; g < c; g++) if (g in f) {
            var h = f[g];
    ...
    _.fb = function (a, b, c) {
        for (var d = a.length, e = _.Ja(a) ? a.split("") : a, f = 0; f < d; f++) if (f in e && !b.call(c, e[f], f, a)) return !1;
        return !0
    };
    _.gb = function (a, b, c) {
        for (var d = a.length, e = _.Ja(a) ? a.split("") : a, f = 0; f < d; f++) if (f in e && b.call(c, e[f], f, a)) return f;
        return -1
    };
    _.ib = function (a, b) {
        b = _.db(a, b);
    ...
    _.ub = function () {
        return -1 != _.tb.toLowerCase().indexOf("webkit")
    };
    _.wb = function (a, b) {
        var c = 0;
    ...
    _.Fb = function () {
        return _.Bb("Safari") && !(_.Eb() || _.Bb("Coast") || _.Bb("Opera") || _.Bb("Edge") || _.Bb("Edg/") || _.Bb("OPR") || _.Db() || _.Bb("Silk") || _.Bb("Android"))
    };
    _.Eb = function () {
    ...
    $b = function () {
        if (_.y.Promise && _.y.Promise.resolve) {
            ...
            var b = dc;
            !_.Ua(_.y.setImmediate) || _.y.Window && _.y.Window.prototype && !_.Bb("Edge") && _.y.Window.prototype.setImmediate == _.y.setImmediate ? (ec || (ec = Wb()), ec(b)) : _.y.setImmediate(b)
        }
    };
    dc = function () {
    ...
    rc = function (a, b) {
        return a === b || null == a && null == b || !(!0 !== a && 1 !== a || !0 !== b && 1 !== b) || !(!1 !== a && 0 !== a || !1 !== b && 0 !== b) ? !0 : _.Sa(a) && _.Sa(b) ? sc(a, b) : !1
    };
    _.D = _.n();
    ...
    _.vc = function (a, b) {
        return _.nc(a.C, b).slice().values()
    };
    _.wc = function (a, b, c) {
        _.nc(a.C, b).push(c)
    };
    ...
    _.Qc = function (a, b) {
        for (var c = _.Pc(void 0, _.L(b)), d = _.Pc(void 0, 0); d < c; ++d) a.push(b[d])
    };
    _.M = function (a) {
        return "number" == typeof a
    };
    ...
    _.Zc = function (a, b) {
        ...
        return function (d) {
            ...
            var e = {}, f;
            for (f in d) if (e[f] = d[f], !b && !a[f]) throw _.Xc(c + "unknown property " + f);
            for (f in a) try {
                var g = a[f](e[f]);
                if (_.t(g) || Object.prototype.hasOwnProperty.call(d, f)) e[f] = g
            } catch (h) {
                throw _.Xc(c + "in property " + f, h);
            }
            return e
        }
    };
    ...
    _.bd = function (a, b, c) {
        return c ? function (d) {
            ...
            try {
                ...
                throw _.Xc("when calling new " + b, e);
            }
        } : function (d) {
            if (d instanceof a) return d;
            throw _.Xc("not an instance of " + b);
        }
    };
    _.cd = function (a) {
        return function (b) {
    ...
    _.dd = function (a) {
        return function (b) {
            if (!_.Sa(b)) throw _.Xc("not an Array");
            return _.Oc(b, function (c, d) {
                try {
                    return a(c)
                } catch (e) {
                    throw _.Xc("at index " + d, e);
                }
            })
        }
    };
    ...
    jd = function (a) {
        ...
        try {
            _.Zc({x: _.id, y: _.id}, !0)(a)
        } catch (b) {
            throw _.Xc("not a Point", b);
        }
    ...
    kd = function (a) {
        ...
        try {
            _.Zc({height: _.id, width: _.id}, !0)(a)
        } catch (b) {
            throw _.Xc("not a Size", b);
        }
    ...
    _.qd = function (a, b) {
        return new _.ld((a.B * b.L - a.m * b.P) / a.D, (-a.A * b.L + a.l * b.P) / a.D)
    };
    _.rd = function (a) {
    ...
    Ad = function (a) {
        var b = _.xd(a) ? a.lat() : a.lat, c = _.xd(a) ? a.lng() : a.lng;
        try {
            return yd({lat: b, lng: c}), _.zd(a)
        } catch (d) {
            throw _.Xc("not a LatLng or LatLngLiteral with finite coordinates", d);
        }
    };
    _.xd = function (a) {
        return a instanceof _.R
    };
    _.zd = function (a) {
        try {
            if (_.xd(a)) return a;
            ...
            throw _.Xc("not a LatLng or LatLngLiteral", b);
        }
    };
    Bd = function (a, b) {
        -180 == a && 180 != b && (a = 180);
    ...
    _.Ed = function (a) {
        return a.isEmpty() ? 0 : _.Cd(a) ? 360 - (a.j - a.l) : a.l - a.j
    };
    Fd = function (a, b) {
        this.j = a;
        this.l = b
    ...
    _.Gd = function (a, b) {
        ...
        if (a) {
            b = b || a;
            var c = _.Lc(a.lat(), -90, 90), d = _.Lc(b.lat(), -90, 90);
            this.na = new Fd(c, d);
            a = a.lng();
            b = b.lng();
    ...
    _.Hd = function (a, b, c, d) {
        return new _.Gd(new _.R(a, b, !0), new _.R(c, d, !0))
    };
    _.Jd = function (a) {
        ...
        try {
            ...
            throw _.Xc("not a LatLngBounds or LatLngBoundsLiteral", b);
        }
    };
    _.Md = function (a) {
        a = a || window.event;
    ...
    Pd = function (a, b) {
        var c = a.__e3_ || {};
        if (b) a = c[b] || {}; else for (b in a = {}, c) _.Kc(a, c[b]);
        return a
    };
    Qd = function (a, b) {
        return function (c) {
    ...
    _.Zd = function (a) {
        return Yd[a] || (Yd[a] = a.substr(0, 1).toUpperCase() + a.substr(1))
    };
    $d = function (a) {
    ...
    _.ee = function () {
        this.W = new _.ae({Ed: (0, _.z)(this.Ed, this), Fd: (0, _.z)(this.Fd, this)})
    };
    _.fe = function (a) {
        return function () {
            return this.get(a)
    ...
    Ae = function (a) {
        ...
        throw _.Xc("not a Geometry or LatLng or LatLngLiteral object");
    };
    Ce = function (a) {
        var b = _.y.document;
        var c = void 0 === c ? Be : c;
    ...
    _.Oe = function (a) {
        ...
        this.j = null;
        try {
            this.j = a.geometry ? Ae(a.geometry) : null
        } catch (b) {
            _.Yc(b)
        }
        this.l = a.properties || {}
    ...
    _.ff = function (a, b, c) {
        function d(w) {
            ...
            try {
                ...
                throw _.Xc('in property "geometry"', F);
            }
            var B = w.properties || {};
        ...
        function e(w) {
            ...
            try {
                ...
                throw _.Xc('in property "coordinates"', E);
            }
            if ("geometrycollection" == x) try {
                return new _.Te(u(w.geometries))
            } catch (E) {
                throw _.Xc('in property "geometries"', E);
            }
        ...
        function h(w) {
            w = k(w);
            return _.zd({lat: w[1], lng: w[0]})
        }

        if (!b) return [];
        c = c || {};
        var k = _.dd(_.id), l = _.dd(h), m = _.dd(g), q = _.dd(function (w) {
            w = l(w);
            if (!w.length) throw _.Xc("contains no elements");
            if (!w[0].equals(w[w.length - 1])) throw _.Xc("first and last positions are not equal");
            return new _.We(w.slice(0, -1))
        }), r = _.dd(f), u = _.dd(e), v = _.dd(d);
        if ("FeatureCollection" == b.type) {
            ...
            try {
                ...
                throw _.Xc('in property "features"', w);
            }
        }
    ...
    _.rf = function (a) {
        function b() {
            e || (e = !0, _.U("infowindow").then(function (f) {
                f.oj(d)
            }))
        }
    ...
    Af = function (a, b) {
        _.Sc(a) ? (this.set("url", a), this.setValues(b)) : this.setValues(a)
    };
    _.Bf = function () {
    ...
    Vf = function (a, b, c, d, e, f) {
        ...
        var g = _.Va(e) ? !!e.capture : !!e, h = Wf(a);
        h || (a[Xf] = h = new Of(a));
        c = h.add(b, c, d, g, f);
        if (c.j) return c;
        d = Yf();
        ...
        d.src = a;
        d.listener = c;
        if (a.addEventListener) Zf || (e = g), void 0 === e && (e = !1), a.addEventListener(b.toString(), d, e); else if (a.attachEvent) a.attachEvent($f(b.toString()), d); else if (a.addListener && a.removeListener) a.addListener(d); else throw Error("addEventListener and attachEvent are unavailable.");
        ag++;
        return c
    };
    Yf = function () {
        var a = bg, b = cg ? function (c) {
    ...
    dg = function (a, b, c, d, e) {
        if (_.Sa(b)) for (var f = 0; f < b.length; f++) dg(a, b[f], c, d, e); else (d = _.Va(d) ? !!d.capture : !!d, c = Tf(c), a && a[Uf]) ? a.A.remove(String(b), c, d, e) : a && (a = Wf(a)) && (b = a.listeners[b.toString()], a = -1, b && (a = Qf(b, c, d, e)), (c = -1 < a ? b[a] : null) && _.eg(c))
    };
    _.eg = function (a) {
        if (!_.Ka(a) && a && !a.Cb) {
            var b = a.src;
            if (b && b[Uf]) _.Pf(b.A, a); else {
                var c = a.type, d = a.j;
    ...
    hg = function (a, b, c, d) {
        var e = !0;
        if (a = Wf(a)) if (b = a.listeners[b.toString()]) for (b = b.concat(), a = 0; a < b.length; a++) {
            var f = b[a];
    ...
    bg = function (a, b) {
        ...
        if (!cg) {
            ...
            if (!(0 > c.keyCode || void 0 != c.returnValue)) {
                ...
                for (e = 0; !b.j && e < c.length; e++) b.currentTarget = c[e], f = hg(c[e], a, !1, b), d = d && f
            }
            return d
        }
    ...
    _.kg = function (a, b) {
        if (!_.Ua(a)) if (a && "function" == typeof a.handleEvent) a = (0, _.z)(a.handleEvent, a); else throw Error("Invalid listener argument");
    ...
    wg = function (a, b) {
        ...
        _.vg[35] && b && b.dE && _.U("util").then(function (f) {
            f.j.m(new _.Hc(b.dE))
        })
    };
    zg = function () {
    ...
    _.Dg = function () {
        ...
        b && Array.isArray(b) && _.vg[15] && b.forEach(function (c) {
            _.M(c) && a.push(c)
        });
        return a
    };
    ...
    Tg = function (a, b, c) {
        (new _.qc(b)).forEach(function (d) {
            var e = d.rc, f = _.mc(a, e);
            if (null != f) if (d.Kd) for (var g = 0; g < f.length; ++g) Sg(f[g], e, d, c); else Sg(f, e, d, c)
        })
    };
    Sg = function (a, b, c, d) {
        if ("m" == c.type) {
    ...
    _.ch = function (a) {
        for (var b; b = a.firstChild;) _.bh(b), a.removeChild(b)
    };
    _.bh = function (a) {
        a = new pg(a);
        try {
    ...
    hh = function (a, b) {
        ...
        if (!_.y.requestAnimationFrame) throw _.U("controls").then(function (l) {
            l.lg(a)
    ...
    jh = function (a, b) {
        _.Vc("The Fusion Tables service will be turned down in December 2019 (see https://support.google.com/fusiontables/answer/9185417). Maps API version 3.37 is the last version that will support FusionTablesLayer.");
        !a || _.Sc(a) || _.M(a) ? (this.set("tableId", a), this.setValues(b)) : this.setValues(a)
    };
    _.kh = _.n();
    lh = function (a) {
        a = a || {};
    ...
    ph = function (a) {
        return a instanceof _.je ? nh(a) : new _.je(oh(a))
    };
    rh = function (a) {
        if (_.Sa(a) || a instanceof _.je) if (0 == _.L(a)) var b = !0; else b = a instanceof _.je ? a.getAt(0) : a[0], b = _.Sa(b) || b instanceof _.je; else b = !1;
    ...
    Ah = function (a) {
        ...
        _.U("map").then(function (c) {
            var d = b.l = c.j, e = b.tileSize || new _.Q(256, 256);
    ...
    Ih = function () {
        var a = Gc(), b = _.I(_.K, 16), c = _.I(_.K, 6), d = _.I(_.K, 13), e = new Eh(131071),
    ...
    Th = function (a, b) {
        ...
        setTimeout(function () {
            _.U("util").then(function (f) {
                f.l.j();
                f.m();
                d && _.U("stats").then(function (g) {
                    g.j.j({ev: "api_alreadyloaded", client: _.I(_.K, 6), key: _.I(_.K, 16)})
                })
            })
        }, 5E3);
        var e = _.I(_.K, 11);
        e && Promise.all([].concat(_.Da(_.vc(_.K, 12))).map(function (f) {
            return _.U(f)
        })).then(function () {
            Sh(e)()
        })
    ...
    Sh = function (a) {
        for (var b = a.split("."), c = window, d = window, e = 0; e < b.length; e++) if (d = c, c = c[b[e]], !c) throw _.Xc(a + " is not a function");
        return function () {
            c.apply(d)
        }
    };
    ...
    _.Vh = function (a, b) {
        ...
        if ("string" === typeof a) {
            if ("IP_BIAS" !== a) throw _.Xc(b + " of type string was invalid: " + a);
            return a
        }
        if (!a || !_.Rc(a)) throw _.Xc("Invalid " + b + ": " + a);
        if (!(a instanceof _.R || a instanceof _.Gd || a instanceof _.sh)) try {
            a = _.Jd(a)
        } catch (c) {
            try {
                a = _.zd(a)
                ...
                try {
                    ...
                    throw _.Xc("Invalid " + b + ": " + JSON.stringify(a));
                }
            }
        }
    ...
    Ea("Promise", function (a) {
        ...
        b.prototype.K = function (g) {
            if (g === this) this.B(new TypeError("A Promise cannot resolve to itself")); else if (g instanceof b) this.fa(g); else {
                a:switch (typeof g) {
                    case "object":
                        var h = null != g;
                        break a;
        ...
        b.race = function (g) {
            return new b(function (h, k) {
                for (var l = _.Ca(g), m = l.next(); !m.done; m = l.next()) d(m.value).Wd(h, k)
            })
        };
        b.all = function (g) {
    ...
    Ea("WeakMap", function (a) {
        ...
        b.prototype.get = function (k) {
            return d(k) && Ia(k, g) ? k[g][this.j] : void 0
        };
        ...
        b.prototype["delete"] = function (k) {
            return d(k) && Ia(k, g) && Ia(k[g], this.j) ? delete k[g][this.j] : !1
        };
        return b
    });
    Ea("Map", function (a) {
        ...
        function d(h, k) {
            ...
            var m = h.l[l];
            if (m && Ia(h.l, l)) for (h = 0; h < m.length; h++) {
                var q = m[h];
    ...
    a:{
        var ri = "", si = function () {
            var a = _.tb;
            if (_.Hf) return /rv:([^\);]+)(\)|;)/.exec(a);
            if (ii) return /Edge\/([\d\.]+)/.exec(a);
            if (_.hi) return /\b(?:MSIE|rv)[: ]([^\);]+)(\)|;)/.exec(a);
            if (_.If) return /WebKit\/(\S+)/.exec(a);
            if (_.gi) return /(?:Version)[ \/]?(\S+)/.exec(a)
        }();
        si && (ri = si ? si[1] : "");
        if (_.hi) {
            var ti = Qb();
            if (null != ti && ti > parseFloat(ri)) {
                qi = String(ti);
                break a
            }
        }
    ...
    _.qc.prototype.forEach = function (a, b) {
        for (var c = {
            ...
            zh: !1,
            value: null
        }, d = 1, e = this.m[0], f = 1, g = 0, h = this.j.length; g < h;) {
            c.rc++;
            g == e && (c.rc = this.m[f++], e = this.m[f++], g += Math.ceil(Math.log10(c.rc + 1)));
            var k = this.j.charCodeAt(g++), l = k & -33, m = c.type = Ei[l];
    ...
    _.Gi = _.gd(_.id, function (a) {
        if (isNaN(a)) throw _.Xc("NaN is not an accepted value");
        return a
    });
    Hi = _.gd(_.id, function (a) {
        if (isFinite(a)) return a;
        throw _.Xc(a + " is not an accepted value");
    });
    ...
    _.sd(0, 0, 0, 0);
    var td = _.Zc({lat: _.id, lng: _.id}, !0), yd = _.Zc({lat: Hi, lng: Hi}, !0);
    ...
    _.p.extend = function (a) {
        this.contains(a) || (this.isEmpty() ? this.j = this.l = a : _.Dd(a, this.j) < _.Dd(this.l, a) ? this.j = a : this.l = a)
    };
    _.p.equals = function (a) {
        return 1E-9 >= Math.abs(a.j - this.j) % 360 + Math.abs(_.Ed(a) - _.Ed(this))
    };
    _.p.center = function () {
    ...
    _.Gd.prototype.toJSON = function () {
        return {south: this.na.j, west: this.ga.j, north: this.na.l, east: this.ga.l}
    };
    ...
    _.S.forward = function (a, b, c) {
        return _.S.addListener(a, b, Rd(b, c))
    };
    ...
    _.T.prototype.set = function (a, b) {
        ...
        a += "";
        var d = Uc(c, a);
        if (d) if (a = d.ub, d = d.Yc, c = "set" + _.Zd(a), d[c]) d[c](b); else d.set(a, b); else this[a] = b, c[a] = null, Xd(this, a)
    };
    ...
    _.T.prototype.bindTo = function (a, b, c, d) {
        ...
        c = (c || a) + "";
        this.unbind(a);
        var e = {Yc: this, ub: a}, f = {Yc: b, ub: c, Jg: e};
        $d(this)[a] = f;
        Wd(b, c)[_.Vd(e)] = e;
        d || Xd(this, a)
    };
    ...
    _.T.prototype.unbindAll = function () {
        var a = (0, _.z)(this.unbind, this), b = $d(this), c;
        for (c in b) a(c)
    };
    ...
    _.ae.prototype.addListener = function (a, b, c) {
        c = c ? {Pg: !1} : null;
        var d = !this.W.length, e = this.W.find(de(a, b));
    ...
    ue.prototype.set = function (a, b) {
        if (null != b && !(b && _.M(b.maxZoom) && b.tileSize && b.tileSize.width && b.tileSize.height && b.getTile && b.getTile.apply)) throw Error("Expected value implementing google.maps.MapType");
...
*/
    ...
    _.Oe.prototype.getProperty = function (a) {
        return Uc(this.l, a)
    };
    ...
    _.Oe.prototype.forEachProperty = function (a) {
    ...
    _.p = Pe.prototype;
    _.p.contains = function (a) {
        return this.j.hasOwnProperty(_.Vd(a))
    };
    ...
    Qe.prototype.forEach = function (a) {
        _.Jc(this.j, a)
    };
    _.A(Re, _.T);
    ...
    Re.prototype.revertStyle = function (a) {
        a ? this.j.reset(_.Vd(a)) : this.j.forEach((0, _.z)(this.j.reset, this.j))
    };
    _.A(_.Te, ye);
    ...
    hf.prototype.controls_changed = function () {
        this.get("controls") && jf(this)
    };
    hf.prototype.drawingMode_changed = function () {
        this.get("drawingMode") && jf(this)
    };
    _.he(hf.prototype, {
        map: _.Vi,
        style: _.jb,
        controls: _.N(_.dd(_.cd(Ti))),
        controlPosition: _.N(_.cd(_.ug)),
        drawingMode: _.N(_.cd(Ti))
    });
    ...
    var bj = _.y.google.maps, cj = Fe.j(), dj = (0, _.z)(cj.ac, cj);
    bj.__gjsload__ = dj;
    _.Jc(bj.modules, dj);
    delete bj.modules;
    var ej = _.Zc({source: _.Ii, webUrl: _.Li, iosDeepLinkId: _.Li});
    var fj = _.gd(_.Zc({placeId: _.Li, query: _.Li, location: _.zd}), function (a) {
        if (a.placeId && a.query) throw _.Xc("cannot set both placeId and query");
        if (!a.placeId && !a.query) throw _.Xc("must set one of placeId or query");
        return a
    });
    _.A(nf, _.T);
    _.he(nf.prototype, {
    ...
    _.p.internalAnchorPoint_changed = pf.prototype.internalPixelOffset_changed = function () {
        var a = this.get("internalAnchorPoint") || _.Ni, b = this.get("internalPixelOffset") || _.Oi;
    ...
    _.p.internalContent_changed = function () {
        this.set("content", kf(this.get("internalContent")))
    };
    ...
    _.A(_.rf, _.T);
    _.he(_.rf.prototype, {
        content: _.fd([_.Li, _.ed($c)]),
        position: _.N(_.zd),
        size: _.N(kd),
        map: _.fd([_.Vi, gj]),
        anchor: _.N(_.bd(_.T, "MVCObject")),
        zIndex: _.Ki
    });
    ...
    vf.prototype.route = function (a, b) {
    ...
    xf.prototype.getElevationAlongPath = function (a, b) {
    ...
    xf.prototype.getElevationForLocations = function (a, b) {
        _.U("elevation").then(function (c) {
    ...
    yf.prototype.geocode = function (a, b) {
    ...
    _.Bf.prototype.fromPointToLatLng = function (a, b) {
        var c = this.j;
        return new _.R(_.hc(2 * Math.atan(Math.exp((a.y - c.y) / -this.A)) - Math.PI / 2), (a.x - c.x) / this.m, void 0 === b ? !1 : b)
    };
    _.jj = Math.sqrt(2);
    _.kj = new _.Bf;
    _.A(_.Cf, _.T);
    ...
    var cg = !_.hi || 9 <= Number(ui), lj = _.hi && !_.Sb("9"), Zf = function () {
    ...
    _.Kf.prototype.preventDefault = function () {
        ...
        var a = this.l;
        if (a.preventDefault) a.preventDefault(); else if (a.returnValue = !1, lj) try {
            if (a.ctrlKey || 112 <= a.keyCode && 123 >= a.keyCode) a.keyCode = -1
        } catch (b) {
        }
    };
    var Uf = "closure_listenable_" + (1E6 * Math.random() | 0), Lf = 0;
    Of.prototype.add = function (a, b, c, d, e) {
        var f = a.toString();
        a = this.listeners[f];
        a || (a = this.listeners[f] = [], this.j++);
        var g = Qf(a, b, d, e);
        -1 < g ? (b = a[g], c || (b.Vd = !1)) : (b = new Mf(b, this.src, f, !!d, e), b.Vd = c, a.push(b));
        return b
    };
    ...
    var Xf = "closure_lm_" + (1E6 * Math.random() | 0), fg = {}, ag = 0,
        ig = "__closure_events_fn_" + (1E9 * Math.random() >>> 0);
    _.A(_.jg, _.Ff);
    _.jg.prototype[Uf] = !0;
    _.jg.prototype.addEventListener = function (a, b, c, d) {
        _.Sf(this, a, b, c, d)
    };
    _.jg.prototype.removeEventListener = function (a, b, c, d) {
        dg(this, a, b, c, d)
    };
    ...
    _.jg.prototype.listen = function (a, b, c, d) {
    ...
    pg.prototype.next = function () {
        do pg.Db.next.call(this); while (-1 == this.l);
        return this.node
    };
    ...
    _.p.changed = function () {
        var a = this.mh(), b = this.yg(), c = Ug(this), d = !!this.Xe();
    ...
    hh.prototype.panBy = function (a, b) {
        var c = this.__gm;
        gh ? _.S.trigger(c, "panby", a, b) : _.U("map").then(function () {
    ...
    hh.prototype.panTo = function (a) {
        var b = this.__gm;
        a = Ad(a);
        gh ? _.S.trigger(b, "panto", a) : _.U("map").then(function () {
            _.S.trigger(b, "panto", a)
        })
    };
    hh.prototype.panTo = hh.prototype.panTo;
    hh.prototype.panToBounds = function (a, b) {
        var c = this.__gm, d = _.Jd(a);
        gh ? _.S.trigger(c, "pantolatlngbounds", d, b) : _.U("map").then(function () {
    ...
    hh.prototype.fitBounds = function (a, b) {
        var c = this, d = _.Jd(a);
        gh ? gh.fitBounds(this, d, b) : _.U("map").then(function (e) {
            e.fitBounds(c, d, b)
        })
    };
    hh.prototype.fitBounds = hh.prototype.fitBounds;
    _.he(hh.prototype, {
        bounds: null, streetView: gj, center: _.N(Ad), zoom: _.Ki, restriction: function (a) {
            ...
            return a
        }, mapTypeId: _.Li, projection: null, heading: _.Ki, tilt: _.Ki, clickableIcons: Ji
    });
    ih.prototype.getMaxZoomAtLatLng = function (a, b) {
    ...
    jh.prototype.changed = function (a) {
        var b = this;
        "suppressInfoWindows" != a && "clickable" != a && _.U("onion").then(function (c) {
            c.j(b)
        })
    };
    ...
    _.sh.prototype.center_changed = function () {
    ...
    th.prototype.setPath = function (a) {
        try {
            this.get("latLngs").setAt(0, ph(a))
        } catch (b) {
            _.Yc(b)
        }
    };
    ...
    _.A(_.uh, th);
    _.uh.prototype.Za = !0;
    _.uh.prototype.getPaths = function () {
        return this.get("latLngs")
    };
    _.uh.prototype.getPaths = _.uh.prototype.getPaths;
    _.uh.prototype.setPaths = function (a) {
        try {
            this.set("latLngs", rh(a))
        } catch (b) {
            _.Yc(b)
        }
    };
    ...
    _.yh.prototype.getPanoramaByLocation = function (a, b, c) {
        this.getPanorama({location: a, radius: b, preference: 50 > (b || 0) ? "best" : "nearest"}, c)
    };
    _.yh.prototype.getPanoramaById = function (a, b) {
    ...
    Ah.prototype.getTile = function (a, b, c) {
        ...
        _.le(this.j, d);
        if (this.l) {
            var e = this.tileSize || new _.Q(256, 256), f = this.m(a, b);
    ...
    Ah.prototype.releaseTile = function (a) {
        a && this.j.contains(a) && (this.j.remove(a), (a = a.__gmimt.od) && a.release())
    };
